...
const addEmployee = async (req, res) => {
  try {
    ...
    if (user) {
      return res
        .status(400)
        .json({ success: false, message: "User already registered in emp" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, error: "Server error in add employee" });
  }
};
...
const updateEmployee = async (req, res) => {
  try {
    ...
    if (!updateEmployee || !updateUser) {
      return res
        .status(404)
        .json({ success: false, error: "Documnent not found" });
    }
    return res
      .status(200)
